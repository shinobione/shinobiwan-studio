...
export function CatalogIntelligenceView() {
  ...
  const embedded = entries.filter(entry => validEmbedding(entry.embedding)).length;
      ...
      {!entries.length ? <div className="catalog-message panel intelligence-empty"><strong>No analysis yet</strong><span>Open a track and choose Analyze with SonicTrace to start building canonical catalog relationships.</span><a className="primary-btn" href="#/catalog">Browse Catalog</a></div> : (
        <>
          <article className="panel sonic-panel c3b-map-panel">
            <div className="intelligence-section-head"><div><span className="eyebrow">CATALOG MAP</span><h3>Position = proximity. Color = family. Zone = neighborhood.</h3><p>The map is recalculated deterministically from canonical persisted embeddings; it is not loaded from SonicTrace standalone memory.</p></div><b>{analysis.projection.length} mapped</b></div>
          ...
          <div className="intelligence-layout c3b-selection-layout">
            ...
            <article className="panel sonic-panel intelligence-neighbors">
              ...
              {selected && <div className="c3b-selected-context"><span>{analysis.zones.groups[analysis.zones.assignments[selected.trackId]]?.label || 'No acoustic zone'}</span><span>{familyFor(selected.trackId)?.label || 'No Neural family'}</span></div>}
