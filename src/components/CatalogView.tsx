  ...
  const filtered = useMemo(() => {
    ...
    return tracks
      ...
      .sort((a, b) => {
        ...
        if (sortMode === 'album') {
          return a.album.title.localeCompare(b.album.title, 'en', { sensitivity: 'base' })
    ...
    <section className="catalog-surface">
      ...
      <div className="catalog-toolbar panel">
        <label className="catalog-search">
          <span>Search</span>
          <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Title, album, genre, mood…" />
        </label>
        <label>
          <span>Album</span>
        ...
        <label>
          ...
          <select value={contentFilter} onChange={event => setContentFilter(event.target.value as ContentFilter)}>
            ...
            <option value="core-complete">Core complete</option>
            <option value="timestamped">Timestamped lyrics</option>
      ...
      {!loading && error && (
        <div className="catalog-message catalog-error panel">
      ...
      {!loading && !error && (
        <>
          ...
          {filtered.length === 0 ? (
            <div className="catalog-message panel">No track matches the current filters.</div>
          ) : (
                  ...
                  <a className="catalog-card panel" key={track.id} href={trackHref(track.id)}>
                    ...
                    <div className="catalog-card-body">
                      <div className="catalog-card-title">
                        ...
                        <small>{releaseLabel(track)}</small>
                      </div>
                      <div className="catalog-tags">
                        {(track.genres.length ? track.genres : ['Unclassified']).slice(0, 3).map(tag => <span key={tag}>{tag}</span>)}
                      </div>
