...
assert.ok(currentBuild >= 95, `Build95 guard requires Build95 or a successor, got Build${currentBuild}.`);
assert.ok(release.includes('build94AncestryMarker'), 'Build95+ must preserve accepted Build94 ancestry.');
assert.ok(release.includes("version: 0.19.16 · build: 94 · codename: 'studio-focus-slice4-phase9-lyrics-validation-transient-retry-truth'"), 'Build94 accepted runtime identity must remain immutable in ancestry.');
if (currentBuild === 95) {
  ...
  assert.ok(release.includes('build95AncestryMarker'), `Build${currentBuild} must preserve accepted Build95 ancestry.`);
...
for (let build = 96; build <= Math.min(currentBuild - 1, 110); build += 1) {
  assert.ok(release.includes(`build${build}AncestryMarker`), `Build${currentBuild} must preserve Build${build} ancestry while inheriting Build95 daily Albums truth.`);
}
...
console.log(`Phase9 Build95 daily Albums resilient-service convergence guard passed through Build${currentBuild}: the actual Albums route still consumes accepted Build85/86/87 mutation truth without widening create/upload/delete scope.`);
