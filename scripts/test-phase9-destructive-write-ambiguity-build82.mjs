...
assert.ok(trackAssets.includes('async function deleteViaFetch('), 'Track delete must use a bounded timeout-aware transport.');
assert.ok(trackAssets.includes('globalThis.setTimeout(() => controller.abort(), 30000)'), 'Track delete transport must have a finite timeout.');
...
console.log('Phase9 Build82 destructive-write ambiguity guard passed: Track and Album asset deletes classify lost responses through private canonical reread with no blind retry.');