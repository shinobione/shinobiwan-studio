...
assert.ok(release.includes("version: '0.9.2'"), 'Studio release version must be 0.9.2.');
assert.ok(release.includes('build: 17'), 'Studio release build must be 17.');
assert.ok(release.includes("codename: 'phase6-embed-runtime-hotfix'"), 'Studio release codename must freeze the Phase 6 embedded runtime hotfix.');
assert.equal(pkg.version, '0.9.2', 'package.json must match Studio 0.9.2.');
...
console.log('Studio 0.9.2 Build 17 preserves prior contracts and consumes the browser-safe LRC Maker 6.3.1 embedded runtime.');
