...
assert.ok(release.includes("version: '0.9.3'"), 'Studio release version must be 0.9.3.');
assert.ok(release.includes('build: 18'), 'Studio release build must be 18.');
assert.ok(release.includes("codename: 'phase6-embed-editor-parity'"), 'Studio release codename must freeze the Phase 6 embedded editor parity hotfix.');
assert.equal(pkg.version, '0.9.3', 'package.json must match Studio 0.9.3.');
...
console.log('Studio 0.9.3 Build 18 preserves prior contracts and consumes LRC Maker 6.3.2 embedded editor parity.');
