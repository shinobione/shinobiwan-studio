...
assert.match(release, /build:\s*(?:69|70|71|72|73|82)/);
assert.match(
  release,
  /codename:\s*'(?:studio-focus-slice4-(?:phase7c-slice1-guided-metadata|phase7c-presmoke-publication-intake-fix|phase7c-duration-evidence-corrective|phase7c-slice2-guided-core-media|phase7c-slice2-status-truth-corrective|phase9-destructive-write-ambiguity-guard))'/,
);
if (/build:\s*82/.test(release)) {
  assert.ok(release.includes('build69AncestryMarker'), 'Build82 must preserve Build69 guided-metadata ancestry.');
  assert.ok(release.includes('build73AncestryMarker'), 'Build82 must preserve accepted Phase7-C program ancestry.');
  assert.ok(release.includes('build81AncestryMarker'), 'Build82 must preserve accepted Build81 ancestry.');
}
...
console.log('Phase 7-C Slice 1 lineage through Build73 remains protected through bounded Build82 Phase9 successor compatibility.');