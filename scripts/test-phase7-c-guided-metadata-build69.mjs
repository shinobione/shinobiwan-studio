...
assert.match(release, /build:\s*(?:69|70|71|72|73|82|83)/);
assert.match(
  release,
  /codename:\s*'(?:studio-focus-slice4-(?:phase7c-slice1-guided-metadata|phase7c-presmoke-publication-intake-fix|phase7c-duration-evidence-corrective|phase7c-slice2-guided-core-media|phase7c-slice2-status-truth-corrective|phase9-destructive-write-ambiguity-guard|phase9-lyrics-save-response-loss-truth))'/,
);
if (/build:\s*(?:82|83)/.test(release)) {
...
if (/build:\s*83/.test(release)) {
  assert.ok(release.includes('build82AncestryMarker'), 'Build83 must preserve accepted Build82 Phase9 ancestry.');
}
...
console.log('Phase 7-C Slice 1 lineage through Build73 remains protected through bounded Build82/Build83 Phase9 successor compatibility.');
