...
assert.match(release, /version:\s*'0\.19\.(?:3|4|5|6|7|8|9|10|11)'/);
assert.match(release, /build:\s*(?:69|70|71|72|73|82|83|84|85|86|87|88|89)/);
assert.match(
  release,
  /codename:\s*'(?:studio-focus-slice4-(?:phase7c-slice1-guided-metadata|phase7c-presmoke-publication-intake-fix|phase7c-duration-evidence-corrective|phase7c-slice2-guided-core-media|phase7c-slice2-status-truth-corrective|phase9-destructive-write-ambiguity-guard|phase9-lyrics-save-response-loss-truth|phase9-sonictrace-save-response-loss-truth|phase9-album-metadata-response-loss-truth|phase9-album-move-response-loss-truth|phase9-album-membership-response-loss-truth|phase9-private-read-transient-retry-truth|phase9-album-private-read-transient-retry-truth))'/,
);
if (/build:\s*(?:82|83|84|85|86|87|88|89)/.test(release)) {
...
if (/build:\s*(?:83|84|85|86|87|88|89)/.test(release)) assert.ok(release.includes('build82AncestryMarker'), 'Build83+ must preserve accepted Build82 Phase9 ancestry.');
if (/build:\s*(?:84|85|86|87|88|89)/.test(release)) assert.ok(release.includes('build83AncestryMarker'), 'Build84+ must preserve accepted Build83 Phase9 ancestry.');
if (/build:\s*(?:85|86|87|88|89)/.test(release)) assert.ok(release.includes('build84AncestryMarker'), 'Build85+ must preserve accepted Build84 Phase9 ancestry.');
if (/build:\s*(?:86|87|88|89)/.test(release)) assert.ok(release.includes('build85AncestryMarker'), 'Build86+ must preserve accepted Build85 Phase9 ancestry.');
if (/build:\s*(?:87|88|89)/.test(release)) assert.ok(release.includes('build86AncestryMarker'), 'Build87+ must preserve accepted Build86 Phase9 ancestry.');
if (/build:\s*(?:88|89)/.test(release)) assert.ok(release.includes('build87AncestryMarker'), 'Build88+ must preserve accepted Build87 Phase9 ancestry.');
if (/build:\s*89/.test(release)) assert.ok(release.includes('build88AncestryMarker'), 'Build89 must preserve accepted Build88 Phase9 ancestry.');
...
console.log('Phase 7-C Slice 1 lineage through Build73 remains protected through bounded Build82-Build89 Phase9 successor compatibility.');
