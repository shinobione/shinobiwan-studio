...
assert.match(release, /build:\s*47/, 'Phase 7-B must be Build 47.');
assert.match(release, /codename:\s*'phase7-b-contextual-receipts'/, 'Phase 7-B codename mismatch.');
assert.equal(pkg.version, '0.17.0', 'package.json must match Studio Build 47 release.');
assert.ok(pkg.scripts['check:phase7']?.includes('test-phase7-workflow-build46.mjs'), 'Build 47 must preserve the Phase 7-A ancestry guard.');
assert.ok(pkg.scripts['check:phase7']?.includes('test-phase7-receipts-build47.mjs'), 'Build 47 receipt guard must be wired into check:phase7.');

for (const marker of [
  "'lrc-maker' | 'sonictrace' | 'track-to-market'",
  "'canonical-write' | 'review-only'",
  "'lyrics-saved' | 'analysis-saved' | 'final-pack-received'",
  'makeContinuationReceipt',
  'parseStandaloneLyricsReceipt',
  "Track-To-Market receipts are review-only",
]) assert.ok(receipts.includes(marker), `Phase 7-B receipt contract is missing ${marker}.`);
...
for (const marker of [
  'handleContinuationReceipt',
  "receipt.trackId !== trackId",
  "receipt.effect === 'review-only'",
  'const canonical = await getCatalogTrack(trackId)',
  "canonical.readSource !== 'private'",
  "status: 'verified'",
  "status: 'verification-error'",
  'Track Manager private catalog reread completed',
  'parseStandaloneLyricsReceipt(event.data)',
  'event.origin !== lrcOrigin',
]) assert.ok(workspace.includes(marker), `Track Workspace receipt verification is missing ${marker}.`);
...
assert.ok(workspace.includes("source: 'sonictrace'"), 'SonicTrace completion must be converted to a typed receipt.');
...
for (const marker of [
  ...
  "data.releaseStatus !== 'final'",
  "source: 'track-to-market'",
  "operation: 'final-pack-received'",
  "effect: 'review-only'",
  'No canonical write is expected or authorized',
]) assert.ok(market.includes(marker), `Track-To-Market receipt boundary is missing ${marker}.`);
...
assert.ok(main.includes("import './phase7-receipts.css'"), 'Phase 7-B receipt stylesheet must be loaded.');
...
console.log('Studio Phase 7-B Build 47 scopes specialist receipts by trackId, canonically rereads real writes and keeps Track-To-Market FINAL review-only.');