...
assert.match(version, /^0\.(?:16|17)\.\d+$/, 'Phase 7-A successors must remain on the authorized Studio Phase 7 release line.');
...
assert.ok(pkg.scripts['check:phase7']?.includes('test-phase7-workflow-build46.mjs'), 'Phase 7-A ancestry guard must remain wired.');
assert.ok(pkg.scripts.build.includes('check:phase7'), 'Production build must execute Phase 7 guards.');
...
for (const marker of [
  ...
  'track.assets.audio', 'track.assets.cover', 'track.assets.lyricsTxt', 'track.timestampsAvailable',
  'track.audioIntelligence.available', 'track.audioIntelligence.outdated',
  'track.publishing.publishable', 'track.publishing.catalogVisible',
  'export function buildTrackWorkflow', 'export function buildCatalogWorkflow',
...
assert.ok(view.includes('This first Phase 7 slice is read-only'), 'Read-only boundary must remain explicit.');
...
assert.ok(styles.includes('@media(prefers-reduced-motion:reduce)'), 'Phase 7-A must preserve reduced-motion accessibility.');

for (const symbol of ['updateTrackMetadata', 'uploadTrackAsset', 'deleteTrackAsset', 'saveSonicTrace', 'createAdminTrack', 'rebuildCatalog', 'updateAlbum']) {
...
assert.doesNotMatch(view, /method\s*:\s*['"](?:POST|PUT|PATCH|DELETE)['"]/i, 'Phase 7-A view must not declare write methods.');
assert.doesNotMatch(workflow, /method\s*:\s*['"](?:POST|PUT|PATCH|DELETE)['"]/i, 'Phase 7-A readiness model must not declare write methods.');

console.log(`Studio Phase 7-A ancestry ${version} Build ${build} keeps workflow orchestration read-only, canonical and deep-linked under the current Phase 7 successor.`);