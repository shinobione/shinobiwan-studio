...
for (const staleVersion of ['6.3.2', '6.3.3']) {
  assert.ok(!embed.includes(`const EMBED_VERSION = '${staleVersion}'`), `Build 20 must not reload stale LRC Maker ${staleVersion}.`);
}
...
console.log('Phase 6 Lyrics contract passed: direct embedded LRC Maker 6.3.4 engine, native click/double-click/Space sync flow, canonical reread hotfix, standalone fallback, canonical timestamps, guarded refresh and 11px readability floor.');
