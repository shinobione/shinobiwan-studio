...
const authorizedStudioFocusSuccessor = /^0\.(?:17|18|19)\.\d+$/.test(version) && codename.startsWith('studio-focus-');
const authorizedPhase7CSuccessor = /^0\.20\.\d+$/.test(version) && codename.startsWith('phase7c-');
assert.ok(c3Line || authorizedPhase7Successor || authorizedStudioFocusSuccessor || authorizedPhase7CSuccessor, `C3 Deep Audio behavior must remain inherited by the accepted C3 line or its explicitly authorized Phase 7 / Studio Focus / Phase 7-C successor, got ${version} / ${codename}.`);
assert.ok(build >= 38, `C3 Studio build must be >= 38, got ${build}.`);

console.log('C3 Deep Audio FULL/PARTIAL/UNAVAILABLE semantics and transport-vs-processing guards passed through the authorized Phase 7-C successor.');
