...
assert.ok(embed.includes("const EMBED_VERSION = '6.3.7';"), 'Studio must cache-bust and consume LRC Maker embed 6.3.7.');
assert.ok(release.includes("version: '0.10.8'") && release.includes('build: 30'), 'Studio release must be v0.10.8 Build 30.');
assert.equal(pkg.version, '0.10.8', 'package.json must match Studio v0.10.8.');

console.log('Studio PHASE UX C2.5-A polish guard passed: Catalog warm/cache+skeleton, Intelligence hover overflow fix, LRC embed 6.3.7 and release markers protected.');
