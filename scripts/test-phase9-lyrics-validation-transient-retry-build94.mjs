...
assert.ok(currentBuild >= 94, `Build94 guard requires Build94 or a successor, got Build${currentBuild}.`);
...
for (let build = 95; build <= Math.min(currentBuild - 1, 110); build += 1) {
  assert.ok(release.includes(`build${build}AncestryMarker`), `Build${currentBuild} must preserve Build${build} ancestry while inheriting Build94 validation truth.`);
}
...
console.log(`Phase9 Build94 Lyrics validation retry guard passed as accepted ancestry under ${pkg.version} / Build${currentBuild}: non-mutating lyrics-validate-v1 retries once only for transient timeout/transport/HTTP failures, while Access/invalid responses remain non-retry and Build83 save stays at zero automatic write retries.`);
