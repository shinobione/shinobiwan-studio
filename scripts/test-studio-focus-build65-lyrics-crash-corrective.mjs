...
assert.ok(['0.19.3', '0.19.4', '0.19.5', '0.19.6', '0.19.7', '0.19.8', '0.19.9'].includes(pkg.version), 'Build65 guard only accepts the validated v0.19.3-v0.19.9 Studio successor line.');
...
if (/build:\s*(?:82|83|84|85|86|87)/.test(release)) assert.ok(release.includes('build81AncestryMarker'), 'Phase9 successors must preserve accepted Build81 ancestry.');
...
console.log(`Studio ${pkg.version} Build65 guard passed through the bounded v0.19.9 Phase9 successor: missing-lyrics presentation remains idempotent and AssetsManager owns guarded TXT upload.`);
